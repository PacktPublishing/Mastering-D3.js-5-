...
function updateSelectedState(e){
	...
	//update bars
	handlePieUpdate(preppedData.pie)
	handleBarUpdate(preppedData.bar)

}

function filterDataForCharts(srcObj){
	let preppedBarData = []
	let preppedPieData = []
	for(key in srcObj){
		if(!['State','Male','Female','TotalBelowPoverty', 'selected'].includes(key)){
 		...
 		if(['Male','Female'].includes(key)){
			...
			preppedPieData.push(obj)
 		}
	}
	return {bar: preppedBarData, pie: preppedPieData};
}
...
function handlePieUpdate(res){
	...
	if(!res.sourceData){
		res = {sourceData: res, pieFn: pieFn}
	}
